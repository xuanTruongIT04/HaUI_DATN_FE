    ...
    filteredCoupons() {
      ...
      if (this.date_start && this.date_end) {
        return this.coupons.filter((coupon) => {
          ...
          if(startDate >= couponStartDate){
            console.log("TEST NGON COUPON NAME: ", coupon.name);
            console.log("1. COUPON START DATE: ", coupon.start_date, "USER START DATE: ", this.date_start);
          }

          if (startDate >= couponStartDate && endDate <= couponEndDate)
            return coupon;
          // else console.log(coupon.name + "SAI ROI");
        });
      } else if (this.date_start && !this.date_end) {
        return this.coupons.filter((coupon) => {
          const couponStartDate = new Date(coupon.start_date);
          if (startDate >= couponStartDate) return coupon;
        });
      } else if (!this.date_start && this.date_end) {
        return this.coupons.filter((coupon) => {
          const couponEndDate = new Date(coupon.end_date);

          if (endDate <= couponEndDate) return coupon;
        });
      }
      return this.coupons;
    },
